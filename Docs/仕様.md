...
- 2026-08-28: **明部の色温度調整**を追加。`convert(highlightWarmth:)`（-1.0…1.0、既定 `0.0`）で、`sdrMode` が `.hdrCurve` / `.hdrML` のときだけ明部の色温度をずらす（正で暖色寄り、負で寒色寄り）。`0.0` のときは従来どおり CoreImage の自動生成ゲインマップ経路をそのまま通り、出力は現行実装とバイト単位で完全一致する。非ゼロのときは、CoreImage の自動生成ゲインマップが 1ch モノクロで per-channel の色差を保持できないため、生転写と同じ ImageIO 低レベル経路（`CGImageDestinationAddAuxiliaryDataInfo`）で 3ch カラーゲインマップを自前生成して添付する経路に切り替わる（新設 `Sources/GainForgeCore/ColorGainMap.swift`）。CLI `--warm <N>`、GUI ツールバー「明部の色温度」スライダーを追加。`GainForgeError.colorGainMapFailed` を追加。SDR 保存（`.sdr`）とゲインマップ付き入力の生転写には一切作用しない。
- 2026-08-28: **明部の色調整に2軸目「ティント」を追加**。`convert(highlightTint:)`（-1.0…1.0、既定 `0.0`、正でマゼンタ寄り・負でグリーン寄り）を `highlightWarmth`（色温度軸）の直後の引数として追加し、写真のホワイトバランスと同じ Temperature / Tint の2軸構成にした。ティント軸は輝度中立（G を下げた分を R/B で取り戻す配分）で明部の明るさを変えない。効く条件・既定 `0.0` 時にバイト一致する経路温存の方針は `highlightWarmth` と同じ（両方が `0.0` のときだけ従来経路）。公開 API `GainForge.isHighlightWarmthAvailable` を、2軸共通の可否判定として **`isHighlightColorShiftAvailable`** にリネーム。CLI `--tint <N>` を追加（`--warm` と併用可）。GUI はツールバーの単一スライダーを廃し、「明部の色」ボタン＋ポップオーバー（色温度・ティントの2本のスライダーとリセットボタン）に変更。
...
- `highlightWarmth`: 明部だけの色温度調整（-1.0…1.0、既定 `0.0` で無効。正で色温度を下げる＝暖色寄り、負で寒色寄り）。`sdrMode` が `.hdrCurve` / `.hdrML` のときだけ効き、`.sdr` と生転写経路には作用しない。`0.0` のときは従来の CoreImage 自動生成ゲインマップ経路をそのまま通り出力は不変。非ゼロのときは 3ch カラーゲインマップを自前生成する経路に切り替わる（詳細は「変換ロジックの要点」内の[明部の色温度・ティント調整](#明部の色温度ティント調整highlightwarmth--highlighttint)を参照）。
- `highlightTint`: 明部だけのティント調整（-1.0…1.0、既定 `0.0` で無効。正でマゼンタ寄り、負でグリーン寄り）。`highlightWarmth`（色温度軸）とは独立した2軸目で、写真のホワイトバランスと同じ Temperature / Tint 構成。**輝度中立**（G を下げた分を R/B で取り戻す配分のため明部の明るさは変わらない）。効く条件・`0.0` 時の挙動温存は `highlightWarmth` と同じで、`highlightWarmth` と `highlightTint` の**両方が `0.0`** のときだけ従来の CoreImage 自動生成ゲインマップ経路をそのまま通り出力は不変。非ゼロなら（どちらか一方でも）3ch カラーゲインマップを自前生成する経路に切り替わる（詳細は同じく[明部の色温度・ティント調整](#明部の色温度ティント調整highlightwarmth--highlighttint)を参照）。
...
`.hdrCurve` / `.hdrML` はどちらも `highlightWarmth`（明部だけの色温度調整）・`highlightTint`（明部だけのティント調整）を受け付ける。写真のホワイトバランスと同じ Temperature / Tint の2軸構成で、`.sdr` には存在しない設定。
...
| `colorGainMapFailed` | 明部の色調整（`highlightWarmth` / `highlightTint` のいずれかが非ゼロ）時、3ch カラーゲインマップの生成・添付・検算に失敗した |
...
| `--warm <N>` | 明部だけの色温度を調整（`-1.0`〜`1.0`、既定 `0`＝無効。正で暖色寄り）。`-x` / `-m` と併用したときだけ効く。`0` のときの出力は従来と同一 |
| `--tint <N>` | 明部だけのティントを調整（`-1.0`〜`1.0`、既定 `0`＝無効。正でマゼンタ寄り、負でグリーン寄り）。輝度中立（明るさは変えず色だけ寄せる）。`--warm` と同時指定できる。`-x` / `-m` と併用したときだけ効く。両方 `0` のときの出力は従来と同一 |
...
- **画面仕様の詳細は [画面仕様.md](画面仕様.md) を参照**（単一ウィンドウ・ファイル一覧テーブル・
  ...
  - ツールバーに「明部の色」ボタンを備え、押すとポップオーバーで「色温度」（寒色⇔暖色）・
    「ティント」（グリーン⇔マゼンタ）の2本のスライダー（各 -1.0〜1.0、既定 0.0）とリセットボタンを
    開く。HDR 補正（`.hdrCurve` / `.hdrML`）で合成する明部だけの色を、色温度とティントの2軸で独立に
    調整する（写真のホワイトバランスと同じ Temperature / Tint 構成）。ボタンのラベルは両軸が 0 なら
    「明部の色」、非ゼロなら現在値を併記。「SDR画像」が「SDRで保存」のとき、変換中、および
    `GainForge.isHighlightColorShiftAvailable` が false のときは無効化。
  - ツールバーに「サイズ」ポップアップ＋数値フィールド（プリセット付き）を備え、書き出し時の縮小
    （元のサイズ / 総画素数 / 横幅 / 縦幅）を選ぶ。縮小のみ・アスペクト比維持。全経路に効く。
  - 設定（品質・出力先・SDR画像の扱い・明部の色温度・明部のティント・リサイズ）は `UserDefaults`
    に永続化する。
...
3. **書き出しは `writeHEIF10Representation(of: SDR, options: [.hdrImage: 合成HDR])`**。SDR ベースと
   ...
   ただし `highlightWarmth` / `highlightTint` のいずれかが非ゼロのときはこの CoreImage 自動生成経路を使わず、
   [明部の色温度・ティント調整](#明部の色温度ティント調整highlightwarmth--highlighttint)で述べる自前の 3ch カラーゲインマップ経路に切り替わる。
...
### 明部の色温度・ティント調整（`highlightWarmth` / `highlightTint`）

`.hdrCurve` / `.hdrML` の合成カーネル（`expandHighlightsKernel` / `expandLUTKernel`）に、明部だけの
per-channel tint を追加したもの（実装は [ColorGainMap.swift](../Sources/GainForgeCore/ColorGainMap.swift)）。
`highlightWarmth`（色温度：寒色⇔暖色）と `highlightTint`（ティント：グリーン⇔マゼンタ）は写真の
ホワイトバランスと同じ Temperature / Tint の**独立した2軸**で、両者を掛け合わせて最終 tint を作る。

1. **色温度軸は既存の明部加重に乗る**。`warmTint = vec3(1 + k·t, 1, 1 − k·t)` を明部加重ゲインへ
   追加で掛ける（`t` は既存の明部加重、`k` は内部係数）。`t` は中間調・暗部でほぼ 0 なので、tint も
   明部だけに効き中間調・暗部は無彩のまま変わらない。UI/CLI の強度 `-1.0…1.0` は
   `HighlightWarmth.coefficient` で内部係数 `k`（強度 1.0 → `k = 0.15`）へ変換する。正で R を持ち上げ
   B を下げる＝暖色寄り。
2. **ティント軸も同じ `t` に乗る**。`tintAxis = vec3(1 + r·m·t, 1 − m·t, 1 + r·m·t)` を追加で掛ける
   （`m` は `HighlightTint.coefficient` が強度 `-1.0…1.0` から変換する内部係数、強度 1.0 → `m = 0.06`）。
   正で G を下げ R/B を持ち上げる＝マゼンタ寄り、負でグリーン寄り。
   `r = HighlightTint.luminanceNeutralRatio = 0.7152 / (0.2126 + 0.0722) ≈ 2.5112`
   （BT.709 の G の輝度加重 ÷ (R+B) の輝度加重）を掛けることで、**G を下げた分をちょうど R/B で
   取り戻す配分**にし、ティント軸単独では明部の輝度 Y が変わらない（**輝度中立**）。実測でも
   ティントを最大まで振っても明部輝度はほぼ不変、かつ色温度軸（R/B 比）とはほぼ直交することを
   確認済み（[SDRのHDR化手法の研究.md](SDRのHDR化手法の研究.md) 参照）。
3. **最終 tint は2軸の積**。`tint = warmTint * tintAxis` を明部加重ゲインへ掛ける。定数は
   `HighlightTint.maxCoefficient` / `HighlightTint.luminanceNeutralRatio`
   （[ColorGainMap.swift](../Sources/GainForgeCore/ColorGainMap.swift)）。
4. **両方が `0.0`（既定）のときだけ従来経路を完全に温存**。`HighlightWarmth.isNeutral &&
   HighlightTint.isNeutral` が真なら CoreImage の `writeHEIF10Representation(of:options:[.hdrImage:])`
   による自動生成（1ch モノクロゲインマップ）をそのまま通る。出力は現行実装とバイト単位で完全一致する。
5. **どちらか一方でも非ゼロなら 3ch カラーゲインマップを自前生成**。CoreImage の自動生成ゲインマップは
   `PixelFormat = 'L008'`（1ch モノクロ）で、per-channel の色差は輝度に平均化されて消えるため、
   tint を保存できない。そこで生転写と同じ ImageIO 低レベル経路
   （`CGImageDestinationAddAuxiliaryDataInfo`）で 3ch(`32BGRA`) のカラーゲインマップを組み立てて添付する
   （`writeColorGainMapHEIC`）。
6. **ゲインマップ metadata はゼロから自作しない**。`CGImageDestinationFinalize` が false になるため、
   CoreImage に 32×32 のダミー画像を一度だけ書かせて正規の metadata を借用し（`ColorGainMap.template`）、
   `ChannelMetadata` を配列タグごと差し替えて `GainMapMin = 0` / `GainMapMax = この画像の実測最大ゲイン`（`measureMaxLog2`）に
   設定し直す（`ColorGainMap.makeMetadata`）。
7. **ベースは `RGBA16` の `CGImage`** として渡す。この経路でも 10bit HEIC（HEVC Main 10）を維持するため。
   元画像の `Depth` プロパティは引き継がない（引き継ぐと 8bit に落ちてしまう）。
8. **検算は 2 段構え**。書き出し後に `hasGainMap`（落とし穴6）に加え、ゲインマップがカラーのまま
   保存されたか（`PixelFormat` が `L008` でないか）も `ColorGainMap.isColorGainMap` で検算する。
   いずれかに失敗すると `GainForgeError.colorGainMapFailed` を投げる。
9. **借用テンプレートを用意できない環境では従来の 1ch 経路へ自動降格**する（`.hdrML` が LUT 欠損時に
   `.hdrCurve` へ降格するのと同じ方針で、環境依存の失敗でバッチを全滅させない）。色温度・ティント調整は
   効かなくなるが変換自体は成功する。可否は 2 軸共通の **`GainForge.isHighlightColorShiftAvailable`**
   （旧 `isHighlightWarmthAvailable` から改称）で公開し、CLI は開始前に一度警告を出し、GUI は「明部の色」
   ボタンを無効化する。
10. **EXIF/GPS/Orientation・PNG の AI 生成タグ引き継ぎ、リサイズとの併用は従来どおり通る**
    （合成前の SDR 縮小 → ベース・ゲインマップとも自動整合）。
11. **既存の低彩度・肌色保護ロジックは色温度・ティントいずれの tint にも適用しない**。ゲイン*量*
    （どれだけ伸ばすか）だけを保護し、tint は保護なしで明部加重にそのまま乗る。白い面が暖色光を
    受けて暖色に寄る、あるいはティントで色かぶりするのは自然な挙動であり、かつ強度は既定 `0.0` で
    ユーザーが明示的に選んだときだけ効くため。実測値・設計判断の詳細は
    [SDRのHDR化手法の研究.md](SDRのHDR化手法の研究.md) を参照。
